...
/**
 * Changes the remark of an existing reader in the address book.
 */
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the reader in the filtered reader list to edit the remark
     * @param remark of the reader to be updated to
     */
...
Create a `RemarkCommandParser` class in the `seedu.smartlib.logic.parser` package. The class must extend the `Parser` interface.
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of reader data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the reader’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a reader.

### Add a new `Remark` class

Create a new `Remark` in `seedu.smartlib.model.reader`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each reader.

Simply add the following to [`seedu.smartlib.ui.ReaderCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
``` java
public PersonCard(Person reader, int displayedIndex) {
    //...
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person readerToEdit = lastShownList.get(index.getZeroBased());
        Person editedReader = new Person(readerToEdit.getName(), readerToEdit.getPhone(), readerToEdit.getEmail(),
                readerToEdit.getAddress(), remark, readerToEdit.getTags());

        model.setPerson(readerToEdit, editedReader);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedReader));
    }
     ...
     * {@code readerToEdit}.
     */
    private String generateSuccessMessage(Person readerToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, readerToEdit);
    }
```
