...
This command allows users of the AddressBook application to add optional remarks to people in their description book and edit it if required. The command should have the following format:
...
Let’s start by creating a new `RemarkCommand` class in the `src/main/java/seedu/description/logic/command` directory.
...
/**
 * Changes the remark of an existing plan in the description book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the plan identified "
            + "by the index number used in the last plan listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the plan in the filtered plan list to edit the remark
     * @param remark of the plan to be updated to
     */
...
Create a `RemarkCommandParser` class in the `seedu.description.logic.parser` package. The class must extend the `Parser` interface.
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of plan data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the plan’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a plan.

### Add a new `Remark` class

Create a new `Remark` in `seedu.description.model.plan`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each plan.
...
:exclamation: You must delete AddressBook’s storage file located at `/data/addressbook.json` before running it! Not doing so will cause AddressBook to default to an empty description book!
...
``` java
public PersonCard(Person plan, int displayedIndex) {
    //...
    remark.setText(plan.getRemark().value);
}
```
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person editedPlan = new Person(planToEdit.getName(), planToEdit.getPhone(), planToEdit.getEmail(),
                planToEdit.getAddress(), remark, planToEdit.getTags());

        model.setPerson(planToEdit, editedPlan);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedPlan));
    }
     ...
     * {@code planToEdit}.
     */
    private String generateSuccessMessage(Person planToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, planToEdit);
    }
```
