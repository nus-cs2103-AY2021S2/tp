...
Create a new `Remark` in `seedu.address.model.person.passenger`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
