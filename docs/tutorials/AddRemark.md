...
This command allows users of the AddressBook application to add optional remarks to people in Link.me and edit it if required. The command should have the following format:
...
/**
 * Changes the remark of an existing person in Link.me.
 */
...
:exclamation: You must delete AddressBook’s storage file located at `/data/linkme.json` before running it! Not doing so will cause AddressBook to default to an empty Link.me!
