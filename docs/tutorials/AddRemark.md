...
This command allows users of the AddressBook application to add optional remarks to people in their location book and edit it if required. The command should have the following format:
...
Let’s start by creating a new `RemarkCommand` class in the `src/main/java/seedu/location/logic/command` directory.
...
``` java
package seedu.location.logic.commands;
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the item identified "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the item in the filtered item list to edit the remark
     * @param remark of the item to be updated to
     */
...
Create a `RemarkCommandParser` class in the `seedu.location.logic.parser` package. The class must extend the `Parser` interface.
...
:exclamation: You must delete AddressBook’s storage file located at `/data/addressbook.json` before running it! Not doing so will cause AddressBook to default to an empty location book!
...
``` java
public PersonCard(Person item, int displayedIndex) {
    //...
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person editedItem = new Person(itemToEdit.getName(), itemToEdit.getPhone(), itemToEdit.getEmail(),
                itemToEdit.getAddress(), remark, itemToEdit.getTags());

        model.setPerson(itemToEdit, editedItem);
     ...
     * {@code itemToEdit}.
     */
    private String generateSuccessMessage(Person itemToEdit) {
