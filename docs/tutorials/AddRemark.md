...
/**
 * Changes the remark of an existing owner in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the owner identified "
            + "by the index number used in the last owner listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the owner in the filtered owner list to edit the remark
     * @param remark of the owner to be updated to
     */
...
Create a new `Remark` in `seedu.address.model.owner`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each owner.

Simply add the following to [`dog.pawbook.ui.EntityCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
Then insert the following into [`main/resources/view/OwnerListCard.fxml`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-12580431f55d7880578aa4c16f249e71).
...
## Modify `Owner` to support a `Remark` field

Since `OwnerCard` displays data from a `Owner`, we need to update `Owner` to get our `Remark` displayed!

### Modify `Owner`

We change the constructor of `Owner` to take a `Remark`. We will also need to define new fields and accessors accordingly to store our new addition.

### Update other usages of `Owner`

Unfortunately, a change to `Owner` will cause other commands to break, you will have to modify these commands to use the updated `Owner`!

<div markdown="span" class="alert alert-primary">

:bulb: Use the `Find Usages` feature in IntelliJ IDEA on the `Owner` class to find these commands.
...
AddressBook stores data by serializing `JsonAdaptedOwner` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedOwner` to work with our new `Owner`!
...
Now that we have finalized the `Owner` class and its dependencies, we can now bind the `Remark` field to the UI.
...
public OwnerCard(Owner owner, int displayedIndex) {
    //...
    remark.setText(owner.getRemark().value);
}
```
...
In this last step, we modify `RemarkCommand#execute()` to change the `Remark` of a `Owner`. Since all fields in a `Owner` are immutable, we create a new instance of a `Owner` with the values that we want and
save it with `Model#setOwner()`.

**`RemarkCommand.java`:**

``` java
//...
    public static final String MESSAGE_ADD_REMARK_SUCCESS = "Added remark to Owner: %1$s";
    public static final String MESSAGE_DELETE_REMARK_SUCCESS = "Removed remark from Owner: %1$s";
//...
    @Override
    public CommandResult execute(Model model) throws CommandException {
        ...
        Owner editedOwner = new Owner(ownerToEdit.getName(), ownerToEdit.getPhone(), ownerToEdit.getEmail(),
                ownerToEdit.getAddress(), remark, ownerToEdit.getTags());

        model.setOwner(ownerToEdit, editedOwner);
        model.updateFilteredOwnerList(PREDICATE_SHOW_ALL_OWNERS);

        return new CommandResult(generateSuccessMessage(editedOwner));
    }
     ...
     * {@code ownerToEdit}.
     */
    private String generateSuccessMessage(Owner ownerToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, ownerToEdit);
    }
```
