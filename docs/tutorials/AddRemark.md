...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the customer identified "
...
Create a `RemarkCommandParser` class in the `seedu.timeforwheels.logic.parser` package. The class must extend the `Parser` interface.
...
Create a new `Remark` in `seedu.timeforwheels.model.customer`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each customer.

Simply add the following to [`seedu.timeforwheels.ui.CustomerCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person editedCustomer = new Person(customerToEdit.getName(), customerToEdit.getPhone(), customerToEdit.getEmail(),
