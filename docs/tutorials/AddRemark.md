...
/**
 * Changes the remark of an existing passenger in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the passenger identified "
            + "by the index number used in the last passenger listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the passenger in the filtered passenger list to edit the remark
     * @param remark of the passenger to be updated to
     */
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of passenger data. We achieve that by working with the `Passenger` model. Each field in a Passenger is implemented as a separate class (e.g. a `Name` object represents the passenger’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a passenger.

### Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.passenger.passenger`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each passenger.

Simply add the following to [`seedu.address.ui.PassengerCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
Then insert the following into [`main/resources/view/PassengerListCard.fxml`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-12580431f55d7880578aa4c16f249e71).

**`PassengerListCard.fxml`:**
...
## Modify `Passenger` to support a `Remark` field

Since `PassengerCard` displays data from a `Passenger`, we need to update `Passenger` to get our `Remark` displayed!

### Modify `Passenger`

We change the constructor of `Passenger` to take a `Remark`. We will also need to define new fields and accessors accordingly to store our new addition.

### Update other usages of `Passenger`

Unfortunately, a change to `Passenger` will cause other commands to break, you will have to modify these commands to use the updated `Passenger`!

<div markdown="span" class="alert alert-primary">

:bulb: Use the `Find Usages` feature in IntelliJ IDEA on the `Passenger` class to find these commands.
...
AddressBook stores data by serializing `JsonAdaptedPassenger` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedPassenger` to work with our new `Passenger`!
...
Now that we have finalized the `Passenger` class and its dependencies, we can now bind the `Remark` field to the UI.
...
public PassengerCard(Passenger passenger, int displayedIndex) {
    //...
    remark.setText(passenger.getRemark().value);
}
```
...
In this last step, we modify `RemarkCommand#execute()` to change the `Remark` of a `Passenger`. Since all fields in a `Passenger` are immutable, we create a new instance of a `Passenger` with the values that we want and
save it with `Model#setPassenger()`.

**`RemarkCommand.java`:**

``` java
//...
    public static final String MESSAGE_ADD_REMARK_SUCCESS = "Added remark to Passenger: %1$s";
    public static final String MESSAGE_DELETE_REMARK_SUCCESS = "Removed remark from Passenger: %1$s";
//...
    @Override
    public CommandResult execute(Model model) throws CommandException {
        ...
        Passenger editedPassenger = new Passenger(passengerToEdit.getName(), passengerToEdit.getPhone(), passengerToEdit.getEmail(),
                passengerToEdit.getAddress(), remark, passengerToEdit.getTags());

        model.setPassenger(passengerToEdit, editedPassenger);
        model.updateFilteredPassengerList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedPassenger));
    }

    /**
     * Generates a command execution success message based on whether the remark is added to or removed from
     * {@code passengerToEdit}.
     */
    private String generateSuccessMessage(Passenger passengerToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, passengerToEdit);
    }
```
