...
/**
 * Changes the remark of an existing entry in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the entry identified "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the entry in the filtered entry list to edit the remark
     * @param remark of the entry to be updated to
     */
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of entry data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the entry’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a entry.

### Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.entry`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each entry.
...
``` java
public PersonCard(Person entry, int displayedIndex) {
    //...
    remark.setText(entry.getRemark().value);
}
```
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person entryToEdit = lastShownList.get(index.getZeroBased());
        Person editedEntry = new Person(entryToEdit.getName(), entryToEdit.getPhone(), entryToEdit.getEmail(),
                entryToEdit.getAddress(), remark, entryToEdit.getTags());

        model.setPerson(entryToEdit, editedEntry);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedEntry));
    }
     ...
     * {@code entryToEdit}.
     */
    private String generateSuccessMessage(Person entryToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, entryToEdit);
    }
```
