...
- Added feature to undo/redo previous commands.
  ...
  * Justification: This feature improves PartyPlanet significantly since a user could make mistakes when writing/using commands, especially since we condensed the available commands to the essentials. As such, PartyPlanet should provide an easily available way to undo this. Keyboard shortcuts were also implemented, e.g. `CTRL + Z` for undo (refer to UG for the shortcuts for `undo` and `redo`)
