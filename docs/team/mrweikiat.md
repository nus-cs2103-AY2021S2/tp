...
* Justification 1: User can view the different priority on the Ui and each priority levels are color coded distinctively from one another
* Justification 2: User can better organise and plan which tasks to do first using the priority levels. 
* Justification 3: Help to identify possible bugs in newly written features.
* Highlights 1: User can use the priority tag feature to categorise different tasks into 3 distinct priority levels, this allows the user to better organise their tasks and be more productive.
* Highlights 2: User can use the `sort` feature to identify tasks that requires more attention so that user can better organise their tasks.
