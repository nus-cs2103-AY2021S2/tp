 ...
 */
public class UniquePersonList implements Iterable<Booking> {
     ...
     */
    public void setPerson(Booking target, Booking editedPerson) {
        ...
        if (!target.isSameBooking(editedPerson) && contains(editedPerson)) {
            throw new DuplicatePersonException();
        }
     ...
     */
    public void setPersons(List<Booking> persons) {
     ...
     */
    private boolean personsAreUnique(List<Booking> persons) {
        for (int i = 0; i < persons.size() - 1; i++) {
            for (int j = i + 1; j < persons.size(); j++) {
                if (persons.get(i).isSameBooking(persons.get(j))) {
                    return false;
                }
            }
        }
