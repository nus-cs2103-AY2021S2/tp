 ...
 */
public class TakenPassengerListPanel extends UiPart<Region> {
    private static final String FXML = "PassengerListPanel.fxml";
    private final Logger logger = LogsCenter.getLogger(TakenPassengerListPanel.class);
     ...
     */
    public TakenPassengerListPanel(ObservableList<Passenger> passengerList) {
        super(FXML);
        passengerListView.setItems(passengerList);
        passengerListView.setCellFactory(listView -> new TakenPassengerListViewCell());
    }

    /**
     * Custom {@code ListCell} that displays the graphics of a {@code Passenger} using a {@code PassengerCard}.
     */
    class TakenPassengerListViewCell extends ListCell<Passenger> {
        @Override
