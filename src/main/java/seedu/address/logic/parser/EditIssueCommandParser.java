...
import seedu.address.logic.commands.EditIssueCommand;
import seedu.address.logic.commands.EditIssueCommand.EditIssueDescriptor;
...
public class EditIssueCommandParser implements Parser<EditIssueCommand> {
    ...
    public EditIssueCommand parse(String args) throws ParseException {
        ...
        if (argMultimap.getValue(PREFIX_ROOM_NO).isPresent()) {
            editPersonDescriptor
                    .setRoomNumber(ParserUtil.parseRoomNumber(argMultimap.getValue(PREFIX_ROOM_NO).get()));
        }
