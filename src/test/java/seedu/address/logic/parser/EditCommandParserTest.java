...
public class EditCommandParserTest {
    ...
    public void parse_invalidValue_failure() {
        ...
        assertParseFailure(parser, "1" + SUBJECT_DESC_MATH + SUBJECT_DESC_CHEM + SUBJECT_EMPTY,
                Subject.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + SUBJECT_DESC_MATH + SUBJECT_EMPTY + SUBJECT_DESC_CHEM,
                Subject.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + SUBJECT_EMPTY + SUBJECT_DESC_MATH + SUBJECT_DESC_CHEM,
                Subject.MESSAGE_CONSTRAINTS);
    ...
    public void parse_allFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_BOB + SUBJECT_DESC_CHEM
                + EMAIL_DESC_AMY + ADDRESS_DESC_AMY + NAME_DESC_AMY + SUBJECT_DESC_MATH + LESSON_DESC_AMY;

        EditPersonDescriptor descriptor = new EditPersonDescriptorBuilder().withName(VALID_NAME_AMY)
                .withPhone(VALID_PHONE_BOB).withEmail(VALID_EMAIL_AMY).withAddress(VALID_ADDRESS_AMY)
                .withSubjects(VALID_SUBJECT_CHEM, VALID_SUBJECT_MATH).withLessons(VALID_LESSON_AMY).build();
    ...
    public void parse_oneFieldSpecified_success() {
        ...
        descriptor = new EditPersonDescriptorBuilder().withLessons(VALID_LESSON_AMY).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
    }
    ...
    public void parse_multipleRepeatedFields_acceptsLast() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_AMY + ADDRESS_DESC_AMY + EMAIL_DESC_AMY
                + SUBJECT_DESC_MATH + PHONE_DESC_AMY + ADDRESS_DESC_AMY + EMAIL_DESC_AMY + SUBJECT_DESC_MATH
                + PHONE_DESC_BOB + ADDRESS_DESC_BOB + LESSON_DESC_AMY + EMAIL_DESC_BOB
                + LESSON_DESC_AMY + LESSON_DESC_BOB + SUBJECT_DESC_CHEM;

        EditPersonDescriptor descriptor = new EditPersonDescriptorBuilder().withPhone(VALID_PHONE_BOB)
                .withEmail(VALID_EMAIL_BOB).withAddress(VALID_ADDRESS_BOB)
                .withSubjects(VALID_SUBJECT_MATH, VALID_SUBJECT_CHEM)
                .withLessons(VALID_LESSON_AMY, VALID_LESSON_BOB).build();
        EditCommand expectedCommand = new EditCommand(targetIndex, descriptor);
